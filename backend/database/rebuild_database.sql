-- 重建数据库脚本 - 包含所有表和字段
-- 执行方式：mysql -u root -p123456 < rebuild_database.sql
-- 注意：此脚本会删除现有数据库并重新创建，请确保已备份数据！

-- 删除现有数据库（如果存在）
DROP DATABASE IF EXISTS `ihc`;

-- 创建数据库
CREATE DATABASE `ihc` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
INSERT INTO `users` (`username`, `email`, `password`, `role`, `nickname`, `createdAt`, `updatedAt`) VALUES
('admin', 'admin@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'admin', '管理员', NOW(), NOW()),
('artist1', 'artist1@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'artist', '艺术家1', NOW(), NOW()),
('user1', 'user1@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'user', '用户1', NOW(), NOW());
